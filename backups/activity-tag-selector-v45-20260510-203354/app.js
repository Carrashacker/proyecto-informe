(function() {
  ...
  var aforoData = null;
  var equationList = [];
  var METSO_VERSION = "v=44-activity-tag-modal";
  ...
  /* Polling: detecta cambios del TAG sin depender de eventos del navegador */
  setInterval(function() {
    var f = document.getElementById("equationForm");
    if (!f || !f.elements.tagKey) return;
    var v = f.elements.tagKey.value;
    if (v === f.elements.tagKey.__lastSeen) return;
    f.elements.tagKey.__lastSeen = v;
    if (typeof populateFlowSelect === "function") populateFlowSelect(v);
    if (typeof autoSetPlantForTag === "function") autoSetPlantForTag(v);
  }, 150);
  ...
  function getTagDisplayByKey(tagKey) {
    ...
    var found = flowCatalog.tags.find(function(tag) { return tag.key === tagKey; });
    return found ? found.display : (tagKey ? tagKey.toUpperCase() : "");
  }

  function syncEquationMode() {
    if (!equationForm) return;
    var offsetInput = equationForm.elements.offset;
    var checkboxes = equationForm.querySelectorAll('input[name="elements"]');
    var hasOffset = !!(offsetInput && String(offsetInput.value).trim() !== "");
    checkboxes.forEach(function(cb) {
      cb.disabled = hasOffset || !currentWeekCanEdit();
      if (hasOffset) cb.checked = false;
    });
    if (offsetInput) offsetInput.disabled = !currentWeekCanEdit();
  }
  ...
  function getActivityPhotos(form) {
    return getExistingPhotoSources(getActivityPhotoGrid(form));
  }
...
loginForm.onsubmit = function(e) {
  ...
  if (emergencyForm) {
    emergencyForm.querySelector(".activity-new").onclick = function() { resetActivityForm(emergencyForm); };
  ...
  if (improvementForm) {
    improvementForm.querySelector(".activity-new").onclick = function() { resetActivityForm(improvementForm); };
  ...
  var equationNewBtn = document.getElementById("equationNewBtn");
  var equationDeleteBtn = document.getElementById("equationDeleteBtn");

  if (equationNewBtn) equationNewBtn.onclick = resetEquationForm;
  if (equationDeleteBtn) equationDeleteBtn.onclick = function() {
    var id = equationForm ? Number(equationForm.elements.id.value) : 0;
    if (!id) return;
  ...
  if (equationForm) {
    ...
    equationForm.addEventListener("change", function(e) {
      if (e.target && e.target.name === "tagKey") {
        populateFlowSelect(e.target.value);
        autoSetPlantForTag(e.target.value);
      }
    });
    ...
    equationForm.elements.offset.oninput = syncEquationMode;
    equationForm.querySelectorAll('input[name="elements"]').forEach(function(cb) {
      cb.onchange = function() {
        if (cb.checked && equationForm.elements.offset) equationForm.elements.offset.value = "";
        syncEquationMode();
      };
    });
  }
  ...
  function renderMaintenance(items) {
    ...
    renderPhotoGrid(card.querySelector(".editable-photos"), selected.photos || [], canEdit);
  ...
  function renderActivities(container, items, type) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      div.className = "record-card" + (isEditing ? " selected-template" : "");
      div.innerHTML =
        ...
        '<button class="soft-button table-action" type="button"' + (canEdit ? "" : " disabled") + '>' + (isEditing ? "Editando..." : "Editar") + '</button>';
      container.appendChild(div);
      div.querySelector("button").onclick = (function(itm, editing) { return function() {
        if (!canEdit) return;
    ...
    if (form) {
      ...
      renderPhotoGrid(getActivityPhotoGrid(form), getActivityPhotos(form), canEdit);
    }
  }
  ...
  function submitActivity(e) {
    ...
    readFilesAsDataUrls(form.elements.photos.files).then(function(newPhotos) {
      ...
      fetch(url, {
        ...
        alert(isEditing ? "Actualizado." : "Agregado.");
        resetActivityForm(form);
        loadReport();
  ...
  function resetActivityForm(form) {
    ...
    renderPhotoGrid(grid, [], currentWeekCanEdit());
    var tagButton = getActivityTagButton(form);
  ...
  function loadActivityIntoForm(form, item) {
    ...
    renderPhotoGrid(getActivityPhotoGrid(form), item.photos || [], currentWeekCanEdit());
    var tagButton = getActivityTagButton(form);
  ...
  function newThermoRecord() {
    promptDialog("Formato dd/mm/yy (ejemplo: 15/05/26)", {
      ...
      fetch("/api/thermography/dates").then(function(r) { return r.json(); }).then(function(dates) {
        ...
        thermoData = { date: date, plants: { colectiva: [], selectiva: [] }, pumps: data_pumps };
        fetch("/api/thermography?date=" + encodeURIComponent(dates.length > 0 ? dates[0] : date)).then(function(r2) { return r2.json(); }).then(function(existing) {
          thermoData = { date: date, plants: existing.plants || { colectiva: [], selectiva: [] }, pumps: existing.pumps || data_pumps };
          renderThermoTables();
        });
      });
    });
  ...
  function loadFlowCatalog() {
    api("GET", "/api/flow-catalog").then(function(r) {
      if (r.ok) {
        ...
        populateTagSelect();
        if (equationForm && equationForm.elements.tagKey.value) populateFlowSelect(equationForm.elements.tagKey.value);
      }
    });
  }
  ...
  function loadEquations() {
    ...
    api("GET", "/api/equations?week=" + encodeURIComponent(week) + "&username=" + encodeURIComponent(currentUser ? currentUser.username : "")).then(function(r) {
      if (weekInput.value.trim() !== week) return;
      if (r.ok) { equationList = r.data.records; renderEquationList(); }
    });
  }
  ...
  function resetEquationForm() {
    if (!equationForm) return;
    equationForm.reset();
    equationForm.elements.id.value = "";
    populateTagSelect();
    populateFlowSelect("");
    equationForm.querySelectorAll('input[name="elements"]').forEach(function(cb) { cb.checked = false; });
    if (equationDeleteBtn) equationDeleteBtn.classList.add("hidden");
    syncEquationMode();
    setFormEnabled(equationForm, currentWeekCanEdit());
  }

  function populateTagSelect() {
    var tagSelect = equationForm ? equationForm.elements.tagKey : null;
    if (!tagSelect || !flowCatalog) return;
    var current = tagSelect.value;
    tagSelect.innerHTML = "<option value=\"\">-- Seleccione equipo --</option>";
    flowCatalog.tags.forEach(function(t) {
      var opt = document.createElement("option");
      opt.value = t.key;
      opt.textContent = t.equipment ? (t.display + " - " + t.equipment) : t.display;
      tagSelect.appendChild(opt);
    });
    if (current) tagSelect.value = current;
  }

  /* COURIER MOLIBDENO (3811-az-601) -> Planta Selectiva. Resto -> Colectiva. */
  function autoSetPlantForTag(tagKey) {
    if (!equationForm || !equationForm.elements.plant) return;
    var planta = (tagKey === "3811-az-601") ? "selectiva" : "colectiva";
    equationForm.elements.plant.value = planta;
    console.log("[METSO] tag=" + tagKey + " -> planta=" + planta);
  }

  /* Handler global accesible via onchange inline en el HTML SSR. Garantiza
     que aunque cualquier listener JS falle, este metodo se ejecute. */
  window.metsoTagChange = function(tagKey) {
    populateFlowSelect(tagKey);
    autoSetPlantForTag(tagKey);
  };

  function populateFlowSelect(tagKey) {
    var flowSelect = equationForm ? equationForm.elements.flow : null;
    if (!flowSelect) return;
    var current = flowSelect.value;
    flowSelect.innerHTML = "<option value=\"\">-- Seleccione flujo --</option>";
    if (!tagKey) return;
    var catalog = flowCatalog || EMBEDDED_FLOW_CATALOG;
    flowCatalog = catalog;
    var flows = (catalog.flows && catalog.flows[tagKey]) || [];
    if (!flows.length) console.warn("[populateFlowSelect] sin flujos para tag", tagKey);
    ...
    if (current && flows.indexOf(current) >= 0) flowSelect.value = current;
  }

  function submitEquationForm(e) {
    e.preventDefault();
    if (!equationForm || !currentWeekCanEdit()) return;
    var id = Number(equationForm.elements.id.value) || 0;
    var tagKey = equationForm.elements.tagKey.value;
    var flowVal = equationForm.elements.flow.value;
    var elements = [];
    equationForm.querySelectorAll('input[name="elements"]').forEach(function(cb) { if (cb.checked) elements.push(cb.value); });
    var offsetValue = equationForm.elements.offset.value;
    var payload = {
      week: weekInput.value,
      username: currentUser.username,
      date: equationForm.elements.date.value,
      time: equationForm.elements.time.value,
      plant: equationForm.elements.plant.value,
      tagKey: tagKey,
      tag: getTagDisplayByKey(tagKey),
      equipment: getTagDisplayByKey(tagKey),
      flow: flowVal,
  ...
  function renderEquationList() {
    ...
    if (equationList.length === 0) {
      ...
      setFormEnabled(equationForm, canEdit);
      syncEquationMode();
      return;
    }
    ...
    equationList.forEach(function(rec) {
      html += '<tr>' +
        ...
        '<td>' + esc(rec.flow) + '</td>' +
        '<td>' + (rec.isPsi ? '<span class="equation-badge psi">PSI</span>' : (rec.elements || []).join(", ")) + '</td>' +
        '<td>' + esc(rec.offset || "") + '</td>' +
        '<td><button class="soft-button table-action" data-eq-id="' + rec.id + '" type="button"' + (canEdit ? '' : ' disabled') + '>Editar</button></td></tr>';
    });
    html += '</tbody></table></div>';
    listEl.innerHTML = html;

    listEl.querySelectorAll("[data-eq-id]").forEach(function(btn) {
      btn.onclick = function() {
        if (!canEdit) return;
        var id = Number(btn.dataset.eqId);
        var rec = equationList.find(function(r) { return r.id === id; });
        if (rec && equationForm) {
          equationForm.elements.id.value = rec.id;
          equationForm.elements.date.value = rec.date || "";
          equationForm.elements.time.value = rec.time || "";
          equationForm.elements.plant.value = rec.plant || "colectiva";
          populateTagSelect();
          equationForm.elements.tagKey.value = rec.tagKey || "";
          populateFlowSelect(rec.tagKey || "");
          equationForm.elements.flow.value = rec.flow || "";
          equationForm.elements.offset.value = rec.offset || "";
          equationForm.querySelectorAll('input[name="elements"]').forEach(function(cb) { cb.checked = !rec.isPsi && (rec.elements || []).indexOf(cb.value) >= 0; });
