(function() {
  ...
  var aforoData = null;
  var equationList = [];
  var METSO_VERSION = "v=57-ecuaciones-rewrite";
...
loginForm.onsubmit = function(e) {
  ...
  if (equationForm) {
    ...
    equationForm.addEventListener("change", function(e) {
      if (e.target && e.target.name === "tagKey") {
        equationsPopulateFlowSelect(e.target.value);
        equationsAutoSetPlant(e.target.value);
      }
    });
  ...
  function submitActivity(e) {
    ...
    var photosPromise = pendingPhotos.length ? Promise.resolve(pendingPhotos) : readFilesAsDataUrls(form.elements.photos.files);
  ...
  function eqResolveTagKey(rawKey) {
    if (!rawKey) return "";
    var upper = rawKey.toUpperCase();
    for (var i = 0; i < EMBEDDED_FLOW_CATALOG.tags.length; i++) {
      if (EMBEDDED_FLOW_CATALOG.tags[i].key === upper ||
          EMBEDDED_FLOW_CATALOG.tags[i].key === rawKey) {
  ...
  function resetEquationForm() {
    ...
    f.reset();
    f.elements.id.value = "";
    f.elements.plant.value = "colectiva";
    equationsPopulateTagSelect();
    ...
    syncEquationMode();
    setFormEnabled(f, currentWeekCanEdit());
  }

  function equationsPopulateTagSelect() {
    var f = document.getElementById("equationForm");
    if (!f) return;
    var tagSelect = f.elements.tagKey;
  ...
  function equationsAutoSetPlant(tagKey) {
    var f = document.getElementById("equationForm");
    if (!f || !f.elements.plant) return;
    var planta = (tagKey && tagKey.toLowerCase() === "3811-az-601") ? "selectiva" : "colectiva";
    f.elements.plant.value = planta;
  }

  function equationsPopulateFlowSelect(tagKey) {
    var f = document.getElementById("equationForm");
    if (!f) return;
    var flowSelect = f.elements.flow;
    if (!flowSelect) return;
    flowSelect.innerHTML = "<option value=\"\">-- Seleccione flujo --</option>";
    if (!tagKey) return;
    var catalog = flowCatalog || EMBEDDED_FLOW_CATALOG;
    var lowerKey = tagKey.toLowerCase();
    var flows = (catalog.flows && catalog.flows[lowerKey]) ||
  ...
  function submitEquationForm(e) {
    e.preventDefault();
    var f = document.getElementById("equationForm");
    if (!f || !currentWeekCanEdit()) return;
    var id = Number(f.elements.id.value) || 0;
    var tagKey = f.elements.tagKey.value;
    var flowVal = f.elements.flow.value;
    var elements = [];
    f.querySelectorAll('input[name="elements"]').forEach(function(cb) { if (cb.checked) elements.push(cb.value); });
    var offsetValue = f.elements.offset.value;
    var payload = {
      week: weekInput.value,
      username: currentUser.username,
      date: f.elements.date.value,
      time: f.elements.time.value,
      plant: f.elements.plant.value,
      tagKey: tagKey,
    ...
    listEl.querySelectorAll("[data-eq-id]").forEach(function(btn) {
      btn.onclick = function() {
        ...
        if (rec) {
          ...
          f.elements.date.value = rec.date || "";
          f.elements.time.value = rec.time || "";
          f.elements.plant.value = rec.plant || "colectiva";
          equationsPopulateTagSelect();
          f.elements.tagKey.value = rec.tagKey || "";
          equationsPopulateFlowSelect(rec.tagKey || "");
          f.elements.flow.value = rec.flow || "";
          f.elements.offset.value = rec.offset || "";
...
function getEquationForm() {
    return document.getElementById("equationForm");
  }

  function submitEquationForm(e) {
    e.preventDefault();
    var f = getEquationForm();
    if (!f || !currentWeekCanEdit()) return;
    var id = Number(f.elements.id.value) || 0;
    var tagKey = f.elements.tagKey.value;
    var flowVal = f.elements.flow.value;
    ...
    var payload = {
      ...
      plant: f.elements.plant.value,
      tagKey: tagKey,
      tag: getTagDisplayByKey(resolved),
      equipment: getTagDisplayByKey(resolved),
      flow: flowVal,
      isPsi: String(offsetValue).trim() !== "",
      elements: elements,
      offset: offsetValue
  ...
  function renderEquationList() {
    ...
    if (equationList.length === 0) {
      listEl.innerHTML = "<p style=\"color:#888;padding:20px\">No hay registros de ecuaciones.</p>";
      setFormEnabled(getEquationForm(), canEdit);
      syncEquationMode();
      return;
    }
    var html = '<div class="equation-table-scroll"><table class="equation-table"><thead><tr><th>Fecha</th><th>Hora</th><th>Planta</th><th>TAG</th><th>Flujo</th><th>Elementos</th><th>Offset</th><th></th></tr></thead><tbody>';
    ...
    listEl.querySelectorAll("[data-eq-id]").forEach(function(btn) {
      btn.onclick = function() {
        ...
        f.elements.tagKey.value = rec.tagKey || "";
        equationsPopulateFlowSelect(rec.tagKey || "");
        equationsAutoSetPlant(rec.tagKey || "");
        f.elements.flow.value = rec.flow || "";
        f.elements.offset.value = rec.offset || "";
        f.querySelectorAll('input[name="elements"]').forEach(function(cb) {
          cb.checked = !rec.isPsi && (rec.elements || []).indexOf(cb.value) >= 0;
