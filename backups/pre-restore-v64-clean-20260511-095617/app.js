(function() {
  ...
  var aforoData = null;
  var equationList = [];
  var METSO_VERSION = "v=59-equation-flow-psi-mode";
  ...
  function syncEquationMode() {
    ...
    var isPsi = isEquationPsiTag(equationForm.elements.tagKey ? equationForm.elements.tagKey.value : "");
...
loginForm.onsubmit = function(e) {
  ...
  window.metsoTagChange = function(tagKey) {
    ...
    var f = document.getElementById("equationForm");
    if (f && f.elements.tagKey && f.elements.tagKey.value !== resolved) f.elements.tagKey.value = resolved;
  ...
  function submitEquationForm(e) {
    ...
    var tagKey = eqResolveTagKey(f.elements.tagKey.value);
    ...
    if (!/^([01][0-9]|2[0-3]):[0-5][0-9]$/.test(String(f.elements.time.value || ""))) {
      alert("Hora invalida. Use formato 24 hrs HH:MM.");
      f.elements.time.focus();
      return;
    }
    ...
    listEl.querySelectorAll("[data-eq-id]").forEach(function(btn) {
      btn.onclick = function() {
        ...
        if (rec) {
          ...
          f.elements.tagKey.value = recTagKey;
